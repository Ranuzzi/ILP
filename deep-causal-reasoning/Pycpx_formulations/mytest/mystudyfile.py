...
options=parsefile("leks_options.txt")
...
#dtargets=parsefile("stitch_interactions_signed.txt")
#sensitivities=parsefile("sensitivities.txt")
...
ds=[ [sol[i][j] for j in list(ddic.itervalues()) ] for i in range(nsol) ]
xnames=list(xdic.iterkeys())
#ynames=list(ydic.iterkeys())
dnames=list(ddic.iterkeys())
Bnames=list(Bdic.iterkeys())
upnames=list(updic.iterkeys())
umnames=list(umdic.iterkeys())
np.savetxt('Ups1.txt',np.concatenate((np.array(upnames,ndmin=2).T,np.array(ups).T),axis=1),delimiter=" ", fmt="%s")
np.savetxt('Ums1.txt',np.concatenate((np.array(umnames,ndmin=2).T,np.array(ums).T),axis=1),delimiter=" ", fmt="%s")
np.savetxt('ds1.txt',np.concatenate((np.array(dnames,ndmin=2).T,np.array(ds).T),axis=1),delimiter=" ", fmt="%s")
#f=open("output.txt", 'w')
...
#targets_act=[0]*len(drugs)
#for i in range(len(drugs)):
 #   targets_act[i]=list()
  #  [targets_act[i].append(dtargets[j][2]) for j in range(len(dtargets)) if dtargets[j][0]==drugs[i] and dtargets[j][1]!="-1"]

#targets_inh=[0]*len(drugs)
#for i in range(len(drugs)):
#    targets_inh[i]=list()
#    [targets_inh[i].append(dtargets[j][2]) for j in range(len(dtargets)) if dtargets[j][0]==drugs[i] and dtargets[j][1]!="1"]

#meas_score=[0]*len(drugs)
#for i in range(len(drugs)):
#    meas_score[i]=sensitivities[1][sensitivities[0].index(drugs[i])]

#pred_score=[0]*len(drugs)
#for i in range(len(drugs)):
#    pred_score[i]=sum([avfx_up[j] for j in range(len(spc)) if spc[j] in targets_inh[i]] +  [avfx_down[j] for j in range(len(spc)) if spc[j] in targets_act[i]])
...
#if len([ pred_score[i] for i in range(len(drugs)) if pred_score[i]>0.0 ]) > 1:
#    regmat=[[drugs[i], float(meas_score[i]), float(pred_score[i])] for i in range(len(drugs)) if pred_score[i]>0 and meas_score[i]!="NaN" ]
